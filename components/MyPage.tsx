...
export default function MyPage() {
        ...
        return item.disabled ? (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-[var(--color-surface-2)]">
          ...
          <Link
            key={item.label}
            href={item.href}
            className="flex items-center justify-between p-4 rounded-xl transition-all hover:bg-[var(--color-surface-hover)] group"
            style={{
              background: 'var(--color-surface)',
              border: '1px solid rgba(255,255,255,0.06)',
            }}
  ...
  if (isDesktop) {
    return (
      <PageLayout title="마이페이지" subtitle="계정 설정 및 활동 내역" width="sm">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Profile Card */}
          <div className="lg:col-span-1">
            <PageCard padding="lg">{profileContent}</PageCard>
          </div>

          {/* Stats & Menu */}
          <div className="lg:col-span-2 space-y-6">
            <PageCard padding="lg">{statsContent}</PageCard>
            {menuContent}
          </div>
        </div>
      </PageLayout>
  ...
  return (
    <PageLayout width="full">
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-3 py-4 border-t border-b border-white/[0.04] my-4">
        {stats.map((stat) => (
          <div key={stat.label} className="text-center">
