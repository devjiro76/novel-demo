...
function ConfirmButtonLabel({ loading, isJoining }: { loading: boolean; isJoining: boolean }) {
  if (loading) {
    return (
      <>
        <Spinner className="size-4" /> 준비 중...
      </>
    );
  }
  return <>{isJoining ? '참가하기' : '대화 시작'}</>;
}
...
export default function RoleSelectScreen({
  ...
  const effectiveCharacterId =
    selectedRole === '__custom__' ? customRoleId.trim() || 'guest' : selectedRole;
  ...
  return (
    <div className="flex h-screen flex-col items-center justify-center overflow-y-auto px-6">
      <div className="slide-up w-full max-w-sm space-y-6 py-8">
        {/* NPC info */}
        <div className="space-y-3 text-center">
          <Avatar
            className="mx-auto size-16 border-2"
          ...
          <div>
            <h2 className={`text-lg font-bold ${npcChar.accentText}`}>{npcChar.fullName}의 방</h2>
        ...
        <div className="space-y-2">
          <label className="block text-xs text-[var(--color-text-secondary)]">역할 선택</label>
          <div className="space-y-1.5">
            {roleOptions.map((role) => (
              <Button
                ...
                className="h-auto w-full justify-start rounded-xl px-4 py-2.5 text-sm"
                style={{
                  background:
        ...
        {selectedRole === '__custom__' && (
          <div className="space-y-2">
            <label className="block text-xs text-[var(--color-text-secondary)]">
              역할 설명{' '}
              <span className="text-[var(--color-text-dim)]">(NPC가 이걸 보고 판단합니다)</span>
            </label>
            <Input
              type="text"
              value={customRoleId}
              ...
              className="h-auto rounded-2xl border-white/[0.06] bg-[var(--color-surface-2)] px-4 py-3 text-sm placeholder:text-[var(--color-text-dim)]"
              style={{
        ...
        {/* Display name input */}
        <div className="space-y-2">
          <label className="block text-xs text-[var(--color-text-secondary)]">표시 이름</label>
          <Input
            type="text"
            value={displayName}
        ...
        <Button
          ...
          className={`h-auto w-full rounded-2xl py-4 text-sm font-bold tracking-wide ${npcChar.btnBg}`}
          style={{
            boxShadow: displayName.trim() ? `0 0 30px rgba(${npcChar.glowRgb},0.2)` : undefined,
          }}
        >
          <ConfirmButtonLabel loading={loading} isJoining={isJoining} />
        </Button>
      </div>
    </div>
  );
