        ...
        {form.tags.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {form.tags.map((tag) => (
              ...
              >
                {tag}
                <span className="ml-0.5 text-[var(--color-text-dim)]">×</span>
              </button>
            ))}
          </div>
        )}
      ...
      <div>
        ...
        <div className="mt-2 flex gap-3">
          {COLOR_PRESETS.map((preset) => (
            ...
            >
              {form.glow === preset.value && <Check className="mx-auto h-5 w-5 text-white" />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <label className="flex cursor-pointer items-center gap-3">
          <div
            ...
            className={`relative h-6 w-11 rounded-full transition-colors ${form.isPublic ? 'bg-[var(--color-brand-primary)]' : 'bg-[var(--color-surface-2)]'}`}
          >
            <span
              className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${form.isPublic ? 'translate-x-6' : 'translate-x-1'}`}
            />
          </div>
        ...
        <label className="flex cursor-pointer items-center gap-3">
          <div
            ...
            className={`relative h-6 w-11 rounded-full transition-colors ${form.isAdult ? 'bg-red-500' : 'bg-[var(--color-surface-2)]'}`}
          >
            <span
              className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${form.isAdult ? 'translate-x-6' : 'translate-x-1'}`}
            />
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-text-muted)]">
              {form.fullName} · {form.age}세 · {form.role || '역할 없음'}
            </p>
          </div>
        </div>
        {form.desc && (
          <p className="text-sm leading-relaxed text-[var(--color-text-secondary)]">{form.desc}</p>
        )}
        {form.tags.length > 0 && (
...
function WizardSuccessScreen({ form }: { form: FormState }) {
  return (
    <PageLayout width="md" className="flex items-center justify-center">
      <div className="animate-scale-in space-y-6 text-center">
        ...
        <div>
          <h2 className="text-gradient mb-2 text-2xl font-bold">{form.name} 생성 완료!</h2>
          <p className="text-[var(--color-text-secondary)]">캐릭터가 성공적으로 만들어졌습니다.</p>
        </div>
        <Link
          href="/create"
          className="inline-flex items-center gap-2 rounded-xl px-8 py-3 text-sm font-semibold text-white transition-all hover:scale-105"
...
export default function CharacterWizard() {
  ...
  if (done) {
    return <WizardSuccessScreen form={form} />;
  }
  ...
  return (
    ...
    >
      ...
      {step === 1 && <Step1Content form={form} set={set} />}
      {step === 2 && (
        <Step2Content
          form={form}
          set={set}
      ...
      {step === 3 && <Step3Content form={form} set={set} />}
      ...
      <div className="mt-8 flex gap-3">
        ...
        {step < 3 ? (
          <button
            type="button"
            onClick={handleNext}
            className="flex-1 rounded-xl py-3.5 text-sm font-semibold text-white transition-all hover:scale-[1.02] active:scale-[0.98]"
            style={{ background: 'var(--color-brand-gradient)' }}
          >
            다음
          </button>
          ...
          <button
            ...
            disabled={submitting}
            className="flex-1 rounded-xl py-3.5 text-sm font-semibold text-white transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
            style={{ background: 'var(--color-brand-gradient)' }}
          >
