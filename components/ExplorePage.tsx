...
export default function ExplorePage({ worlds }: ExplorePageProps) {
  ...
  // Extract all tags
  const allTags = Array.from(new Set(worlds.flatMap((w) => w.tags))).sort((a, b) =>
    a.localeCompare(b, 'ko'),
  );

  // Extract all characters
  const allCharacters: Array<{ char: WorldCharacter; world: WorldCardData; slug: string }> =
  ...
  if (selectedTag !== 'all') {
    filteredCharacters = filteredCharacters.filter(({ world }) => world.tags.includes(selectedTag));
  }
  ...
  const filterSection = (
    <div className="scrollbar-hide mb-4 flex gap-2 overflow-x-auto pb-2">
      <button
        ...
        style={{
          background:
            selectedTag === 'all' ? 'var(--color-brand-gradient)' : 'var(--color-surface)',
      ...
      {allTags.map((tag) => (
        <button
          ...
          style={{
            background:
              selectedTag === tag ? 'var(--color-brand-gradient)' : 'var(--color-surface)',
  ...
  const searchBar = (
    <div className="relative mb-4">
      ...
      <input
        ...
        placeholder="캐릭터 검색..."
        className="w-full rounded-xl border border-white/[0.06] bg-[var(--color-surface)] py-3 pr-4 pl-10 text-sm text-[var(--color-text)] transition-colors placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-brand-primary)]/30 focus:outline-none"
      />
    </div>
  );
  ...
  const characterGrid =
    sortedCharacters.length === 0 ? (
      ...
      <div className="flex flex-wrap gap-3 lg:gap-4">
        {sortedCharacters.map(({ char, world: _world, slug }, index) => (
          <CharacterCard
            key={char.id}
            charId={char.id}
            name={char.name}
  ...
  return (
    <PageLayout title="탐색" subtitle="다양한 캐릭터를 발견하세요" width="lg" showBackButton>
      {content}
    </PageLayout>
  );
}
