 ...
 * Collects structured entries per-request, outputs JSONL to console,
 * and returns accumulated data as `_debug` in API responses.
 */
...
export class DebugLog {
  ...
  /** Add an entry and log it as JSONL to console */
  add(type: DebugEntry['type'], data: Record<string, unknown>, durationMs?: number) {
    ...
    // JSONL to console — grep-friendly
  ...
  /** Log emotion state before/after for qualitative review */
  logEmotionChange(
    characterId: string,
    displayName: string,
  ...
  /** Get all entries for embedding in response */
  getEntries(): DebugEntry[] {
    return this.entries;
  }

  /** Summary entry with total request time */
  finalize(): DebugEntry[] {
    this.add('request_summary', {
      totalMs: Date.now() - this.requestStart,
      entryCount: this.entries.length - 1, // exclude this one
    });
    return this.entries;
  }
}
